...
const AccountLedgerViewPage = () => {
  ...
  const {
    ...
    queryFn: () =>
      id && from && to
        ? GetLedgerEntriesByAccountAndDateRange(id, from, to)
  ...
  console.log("Fetched ledger entries:", ledgerData);

  // Ensure ledgerData exists before sorting
  const ledgerEntries: LedgerEntry[] = ledgerData?.data?.ledgers || [];
  ...
  // Compute balanceBefore directly
  const balanceBefore =
    sortedEntries.length > 0
      ? Number(sortedEntries[0].previousBalance) || 0
      : 0;
  ...
  const { totalDebit, totalCredit } = sortedEntries.reduce(
    (totals, entry) => {
      const amount = Number(entry.amount);
      if (entry.transactionType.toUpperCase() === "DEBIT") {
      ...
      <Card className="w-full max-w-4xl mx-auto mt-16 p-6 shadow-lg">
        <CardHeader>
          <CardTitle className="text-2xl font-semibold text-center">
            Account Ledger:{" "}
            {sortedEntries.length > 0
              ? sortedEntries[0].account.name.trim()
              : "Unknown Account"}
          </CardTitle>
        </CardHeader>
        <CardContent>
              ...
              {sortedEntries.map((entry) => {
                ...
                return (
                  <TableRow key={entry.id}>
                    ...
                    <TableCell className="text-center">
                      {entry.transactionType.toUpperCase() === "DEBIT"
                        ? amount.toLocaleString()
                        : "-"}
                    </TableCell>
                    <TableCell className="text-center">
                      {entry.transactionType.toUpperCase() === "CREDIT"
                        ? amount.toLocaleString()
                        : "-"}
                    </TableCell>
                    <TableCell className="text-center">
